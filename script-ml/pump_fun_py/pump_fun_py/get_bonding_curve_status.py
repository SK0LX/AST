...
# Constants
RPC_URL: Final[str] = "https://mainnet.helius-rpc.com/?api-key=0c503158-dced-43ab-9330-2d1a30098b95"
TOKEN_MINT: Final[str] = "G4rUCNW7xS1ALwJ72XDcojvNbRayNLKbeTGJdHGRpump"
...
async def get_curve_progress(mint: str, ctx, delay) -> float:
    mint_pubkey: Pubkey = Pubkey.from_string(mint)
    ...
    while True:
        try:
            ...
            price = (state['virtual_sol_reserves']) / (state['virtual_token_reserves'])
            return price
...
def print_curve_status(state: dict) -> None:
    ...
    print("=" * 30)
    print(f"price: {(state['virtual_sol_reserves']) / (state['virtual_token_reserves'])}")
