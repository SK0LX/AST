...
async def trade_wrapper(user_wallet, pool, trade_history):
    ...
    def patched_update_balance(change):
        ...
        logger.info(f"Balance updated for {public_key[:8]}: {format_wallet_info(public_key, wallet_store[public_key])}")
    ...
    try:
        async for message in websocket:
            ...
            if action == "wallet:added":
                ...
                logger.info(f"Wallet added: {format_wallet_info(public_key, wallet_store[public_key])}")
                ...
                if wallet and public_key not in trade_tasks:
                    ...
                    logger.info(f"Bot started for {public_key[:8]}: {format_wallet_info(public_key, wallet_store[public_key])}")
                ...
                logger.info(f"Bot stopped for {public_key[:8]}: {format_wallet_info(public_key, wallet_store[public_key])}")
            ...
            logger.info(f"Current wallet store: {format_wallet_store(wallet_store)}")
