...
async def listen_for_new_tokens(ctx):
    reconnect_delay = 5  # Начальная задержка для переподключения (секунды)
    max_reconnect_delay = 60  # Максимальная задержка (секунды)

    while True:
        try:
            print(f"Connecting to WebSocket: {WS_URL}")
            async with websockets.connect(WS_URL, ping_interval=20, ping_timeout=60) as ws:
                # Сохраняем WebSocket в контексте, если нужно
                ctx.ws = ws
                ...
                while True:
                    try:
                        ...
                        price = ((virtual_sol) / (virtual_tokens)) if virtual_tokens > 0 else 0
            ...
            print(f"WebSocket connection failed: {e}. Reconnecting in {reconnect_delay} seconds...")
            await asyncio.sleep(reconnect_delay)
            reconnect_delay = min(reconnect_delay * 2, max_reconnect_delay)  # Экспоненциальная задержка
        except Exception as e:
            print(f"Failed to connect to WebSocket: {e}. Reconnecting in {reconnect_delay} seconds...")
            await asyncio.sleep(reconnect_delay)
            reconnect_delay = min(reconnect_delay * 2, max_reconnect_delay)
        finally:
            # Сбрасываем задержку после успешного соединения
            reconnect_delay = 5
