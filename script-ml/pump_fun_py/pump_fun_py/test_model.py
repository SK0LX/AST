...
def clean_data(args, delta_get_price, buy_price):
    ...
    return np.array([[initial_buy, market_cap, price, sol_amount, delta_get_price, buy_price]])
...
async def execute_trade(payer_keypair, args, mint, bonding_curve, position_size, trade_history):
    ...
    token_data = token_data = clean_data(args, delta_get_price, buy_price)
    print(token_data)
    ...
    if prediction == 1:
        ...
        # Новый клиент для каждой итераци
    else:
